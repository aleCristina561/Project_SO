...
#define HUNTS_DIRECTORY "hunts" // Directory for storing hunts
...
void log_operation(const char *hunt, const char *message) { 
    char log_path[256]; 
    snprintf(log_path, sizeof(log_path), "%s/%s/%s", HUNTS_DIRECTORY, hunt, LOG_FILENAME); 
...
int add_treasure(const char *hunt_id) { 
	struct stat st;
    if (stat(HUNTS_DIRECTORY, &st) == -1) { 
        if (mkdir(HUNTS_DIRECTORY, 0755) != 0) {
    ...
    char dir_path[128]; 
    snprintf(dir_path, sizeof(dir_path), "%s/%s", HUNTS_DIRECTORY, hunt_id); 
    mkdir(dir_path, 0755);

    char file_path[256];
    snprintf(file_path, sizeof(file_path), "%s/%s/%s", HUNTS_DIRECTORY, hunt_id, DATA_FILENAME);
...
void list_hunts() {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        char data_path[256];
        snprintf(data_path, sizeof(data_path), "%s/%s/%s", HUNTS_DIRECTORY, entry->d_name, DATA_FILENAME);
        int fd = open(data_path, O_RDONLY);
        if (fd < 0) {
            continue; // skip if no treasures.dat
        }
...
int remove_treasure(const char *hunt_id, int id) { 
    ...
    snprintf(file_path, sizeof(file_path), "%s/%s/%s", HUNTS_DIRECTORY, hunt_id, DATA_FILENAME); 
    snprintf(temp_path, sizeof(temp_path), "%s/%s/temp.dat", HUNTS_DIRECTORY, hunt_id);
...
int remove_hunt(const char *hunt_id) { 
    ...
    snprintf(log_symlink, sizeof(log_symlink), "%s/%s/%s", HUNTS_DIRECTORY, hunt_id, LOG_FILENAME);
    unlink(log_symlink);

    char data_file[256], log_file[256];
    snprintf(data_file, sizeof(data_file), "%s/%s/%s", HUNTS_DIRECTORY, hunt_id, DATA_FILENAME);
    snprintf(log_file, sizeof(log_file), "%s/%s/%s", HUNTS_DIRECTORY, hunt_id, LOG_FILENAME);
    unlink(data_file);
    unlink(log_file);
    rmdir(dir_path);
